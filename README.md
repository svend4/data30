...
## Повторяющиеся темы и канонические документы
- **План проекта / Project Plan** → `docs/project-plan.md`.
- **Версионирование API** → раздел "API Versioning" в этом README.
- **Макросы** → `docs/macros/spec.md` и `docs/macros/guide.md` (схема: `docs/macro_schema.json`).
- **Модерация и анти-спам** → объединенный раздел "Механизм модерации и анти-спама" в этом README.

## План проекта
Канонический план проекта хранится в `docs/project-plan.md`.
...
- Логирование решений модератора и пересчет рейтингов с исключением отклоненных отзывов.
- Репутационные коэффициенты для пользователей с историей подтвержденных отзывов.

### 5. Алгоритм ранжирования по сценариям
