...
## 6. Аналитика и метрики (инициатива)
1. Определить набор ключевых метрик (growth, LTV, ARPU, churn, CAC).
2. Собрать исторические данные и построить базовые модели прогноза (cohort/retention).
3. Настроить регулярную отчетность и дашборды.
4. Ввести target ranges и сценарные прогнозы (baseline/optimistic/conservative).
5. Определить владельцев метрик и периодичность пересмотра.
